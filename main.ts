		...
		return [
			{
				...
				render: (setting) => {
					...
					setting.settingEl.createDiv({
						text: '1. Create an OAuth client at Google Cloud Console (APIs & Services > Credentials > Create OAuth client ID > Desktop app). 2. Enable the Google Drive API in your project. 3. Copy the client ID and client secret below.',
					});
				},
			},
			{
			...
			{
				name: 'Refresh token',
				desc: 'Obtain via OAuth authorization flow using your Client ID. The refresh token is exchanged directly with Google (oauth2.googleapis.com).',
				control: {
					type: 'text',
					key: 'refreshToken',
